...
public:
    ...
    int stoneGameV(vector<int>& stoneValue) {
        ...
        for(int ln=1;ln<=n;ln++){ //  区间dp 设置区间的长度
            for(int i=1;i+ln <= n+1;i++){ // 设置区间的其实位置
                int k = i+ln-1;
                for(int mid=i;mid<k;i++){ // 枚举区间中的中间点的
                    int sum1 = summ[mid] - summ[i-1], sum2 = summ[k] - summ[mid-1];
                    if(sum1 > sum2)
                        dp[i][k] = max(dp[i][k], dp[mid][k]+sum2);
                    else
                        dp[i][k] = max(dp[i][k], dp[i][mid]+sum2); 
