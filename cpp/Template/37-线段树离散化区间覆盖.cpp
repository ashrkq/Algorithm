...
// 使用左封右开的区间查询。区间颜色l-r就是l-r来标记标记的使用的第几个区间到
// 第几个区间
...
void update(int l, int r, int node, int ql, int qr, int value)
{
    ...
    if (ql < mid)
        update(l, mid, node << 1, ql, qr, value);
    if (qr > mid)
        update(mid, r, node << 1 | 1, ql, qr, value);
}
...
int main()
{
    ...
    int m = 1;
    for (auto id : paper)
        update(1, num, 1, idx[id.first], idx[id.second], m++);
    query(1, num, 1);
    printf("%lu\n", colors.size());
    return 0;
}