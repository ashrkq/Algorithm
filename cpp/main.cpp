...
void build(int l, int r, int idx){
    if( l == r){ scanf("%d", tree + idx); return; }
    int mid = ( l + r ) >> 1;
    build(l, mid, (idx << 1) + 1);
...
int query(int l, int r,int idx, int ql, int qr){
    ...
    int mid = (l + r) >> 1;
    if(qr <= mid ) return query(l, mid, (idx << 1) + 1, ql, qr);
    if(ql > mid) return query(mid + 1, r, (idx << 1) + 2, ql, qr);
    return min( query(l, mid, (idx << 1) + 1, ql, mid), query(mid + 1, r, (idx << 1) + 2, mid+1, qr) );
}
void update(int l, int r, int idx, int x, int value){
    if(l == r){ tree[idx] = value; return; }
    int mid = (l+r) >> 1;
    if(mid >= x) update(l, mid, (idx << 1) + 1, x, value);
    else update(mid + 1, r, (idx << 1) + 2, x, value);
    tree[idx] = min(tree[(idx << 1)+1], tree[(idx << 1) + 2]);
}
